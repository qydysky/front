...
func (localDealer) Deal(ctx context.Context, reqId uint32, w http.ResponseWriter, r *http.Request, routePath string, chosenBack *Back, logger Logger, blocksi pslice.BlocksI[byte]) error {
	var (
		...
		opT       = time.Now()
		logFormat = "%v %v %v%v > %v local %v %v %v %v"
	)
	...
	if !pfile.New(path, 0, true).IsExist() {
		logger.Warn(`W:`, fmt.Sprintf(logFormat, reqId, r.RemoteAddr, chosenBack.route.config.Addr, routePath, chosenBack.Name, "Err", r.RequestURI, errors.New("NotExist "+path), time.Since(opT)))
